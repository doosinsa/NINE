...
async function main() {
  ...
  const target = new URL("/api/prices/collect", options.baseUrl);
  const body = {
    date: options.date,
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Price collection failed with HTTP ${response.status}: ${summarizeText(text)}`);
    }

    if (!json || json.ok !== true) {
      throw new Error(`Price collection returned an error envelope: ${summarizeText(text)}`);
    }
...
function printSummary(origin, data) {
  const sources = Array.isArray(data?.prices)
    ? [...new Set(data.prices.map((price) => price.dataSource).filter(Boolean))].sort()
    : [];
...
function printHelp() {
  ...
  NINE_BASE_URL=http://127.0.0.1:3000 npm run collect:prices -- --date 2026-05-13

Options:
  --base-url     Local worker app URL. Defaults to NINE_BASE_URL or http://127.0.0.1:3000.
  --date         Collection date in YYYY-MM-DD. Defaults to current KST date.
...
main().catch((error) => {
  console.error(error.name === "AbortError" ? "Price collection timed out." : error.message);
  process.exit(1);
});
